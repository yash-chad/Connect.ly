...
router.get('/', async (req, res) => {
  const posts = await Post.find({ "isBook": !true , "isBook": !true}).sort({ timestamp: -1 });
  res.status(200).json(posts);
});

router.get('/books', async (req, res) => {
  const posts = await Post.find({ "isBook": true}).sort({ timestamp: -1 });
  res.status(200).json(posts);
});

router.get('/internships', async (req, res) => {
  const posts = await Post.find({ "isInternShip": true}).sort({ timestamp: -1 })
 // console.log(posts)
  res.status(200).json(posts);
});
